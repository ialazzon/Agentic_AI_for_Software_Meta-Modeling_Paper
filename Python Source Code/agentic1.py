...
("user",
 ...
 "Return ONLY the YAML content.")
    ...
    xmi_path = r".\models2\generated_model.model"
...
def route_after_ocl(state: ModelState) -> str:
    """End if OCL passed, otherwise send back to generate_xmi to fix violations."""
    if not state.get("ocl_errors", ""):
        return "end"
    
    if state.get("ocl_iteration", 0) >= state.get("max_iterations", 5):
        print(
            f"\n[WARN] OCL fix limit reached after {state['ocl_iteration']} attempts. "
            f"Ending with unresolved violations:\n{state['ocl_errors']}"
        )
...
graph.add_edge("load_ecore", "generate_xmi")
...
# RUN FUNCTION
# ----------------------------
def run_agent(ecore_path: str, spec: str):
    ...
    return state.values  # <-- return the final state dict
...
if __name__ == "__main__":
    ...
    spec = input("Describe model instance in natural language: ")

    start = time.time()
    result = run_agent(ecore_path, spec)
    end = time.time()
